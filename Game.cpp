...
namespace game {
    ...
    */
    void Game::Draw(float x, float y, const Texture* tex, RECT *rect, float alpha, int sprite_width, int sprite_height) {
        if (tex == NULL)
            return;
    ...
    */
    void Game::Load(LPCWSTR gameFile) {
        ...
        while (f.getline(str, MAX_GAME_LINE)) {
            ...
            if (line == "[TEXTURES]") {
                section = GAME_FILE_SECTION_TEXTURES;
                continue;
            }
            if (line == "[SCENES]") {
                section = GAME_FILE_SECTION_SCENES;
                continue;
            }
            if (line[0] == '[') {
